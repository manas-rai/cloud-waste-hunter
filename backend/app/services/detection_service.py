...
class DetectionService:
    ...
    async def run_scan(
        ...
        try:
            ...
            total_savings = sum(
                d.get("estimated_monthly_savings_inr", 0)
    ...
    async def _collect_detections(
        ...
        if ResourceType.EC2_INSTANCE in resource_types:
            try:
                ...
                logger.error("EC2 detection failed", error=str(e))
        ...
        if ResourceType.EBS_VOLUME in resource_types:
            try:
                ...
                logger.error("EBS detection failed", error=str(e))
        ...
        if ResourceType.EBS_SNAPSHOT in resource_types:
            try:
                ...
                logger.error("Snapshot detection failed", error=str(e))
    ...
    async def _detect_ebs_unattached(self, client_factory: AWSClientFactory) -> List[Dict]:
    ...
    async def _detect_old_snapshots(self, client_factory: AWSClientFactory) -> List[Dict]:
